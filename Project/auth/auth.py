...
def register():
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            result = DB.insertOne("INSERT INTO IS601_Users (email, username, password) VALUES (%s, %s, %s)", email,
                                  username, hash)
            if result.status:
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if is_valid:
            try:
                result = DB.selectOne(
                    "SELECT id, email, username, password FROM IS601_Users where email= %(email)s or username=%(email)s",
                    {"email": email})
    ...
    if form.validate_on_submit():
        ...
        if is_valid:
            try:  # update email, username (this will trigger if nothing changed but it's fine)
                result = DB.update("UPDATE IS601_Users SET email = %s, username = %s WHERE id = %s", email, username,
                                   user_id)
                if result.status:
